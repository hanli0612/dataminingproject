...
if __name__ == "__main__":
    ...
    for item in total_business:
        if "Food Trucks" in item['categories'] or "Delis" in item['categories'] or 'Street Vendors' in item['categories'] or 'Food Stands' in item[
            ...
            if item['open']:
                truck_num[item['Demo_Cluster']] += 1
                truck_review[item['Demo_Cluster']] += item['review_count']
                #Checkin Info
                    ...
                    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
                        for hour in range(0, 23):
                            if day in item['checkin_info'].keys():
                                if str(hour) in item['checkin_info'][day].keys():
                                    if 0 <= hour and 4 > hour:
                                        food_0am_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 4 <= hour and 8 > hour:
                                        food_4am_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 8 <= hour and 12 > hour:
                                        food_8am_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 12 <= hour and 16 > hour:
                                        food_12pm_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 16 <= hour and 20 > hour:
                                        food_4pm_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 20 <= hour and 24 > hour:
                                        food_8pm_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]


        else:
            if item['open']:
                nofood_review[item['Demo_Cluster']] += item['review_count']
                if 0 < item['stars'] and 1 >= item['stars']:
                    nofood_1star[item['Demo_Cluster']] += 1
                elif 1 < item['stars'] and 2 >= item['stars']:
                    nofood_2star[item['Demo_Cluster']] += 1
                elif 2 < item['stars'] and 3 >= item['stars']:
                    nofood_3star[item['Demo_Cluster']] += 1
                elif 3 < item['stars'] and 4 >= item['stars']:
                    nofood_4star[item['Demo_Cluster']] += 1
                else:
                    nofood_5star[item['Demo_Cluster']] += 1

                if "Price Range" in item['attributes'].keys():
                    if 0 < item['attributes']['Price Range'] and 1 >= item['attributes']['Price Range']:
                        nofood_1price[item['Demo_Cluster']] += 1
                    elif 1 < item['attributes']['Price Range'] and 2 >= item['attributes']['Price Range']:
                        nofood_2price[item['Demo_Cluster']] += 1
                    elif 2 < item['attributes']['Price Range'] and 3 >= item['attributes']['Price Range']:
                        nofood_3price[item['Demo_Cluster']] += 1
                    else:
                        nofood_4price[item['Demo_Cluster']] += 1
                    ...
                    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
                        for hour in range(0, 23):
                            if day in item['checkin_info'].keys():
                                if str(hour) in item['checkin_info'][day].keys():
                                    if 0 <= hour and 4 > hour:
                                        nofood_0am_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 4 <= hour and 8 > hour:
                                        nofood_4am_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 8 <= hour and 12 > hour:
                                        nofood_8am_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 12 <= hour and 16 > hour:
                                        nofood_12pm_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 16 <= hour and 20 > hour:
                                        nofood_4pm_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
                                    if 20 <= hour and 24 > hour:
                                        nofood_8pm_checkin[item['Demo_Cluster']] += item['checkin_info'][day][str(hour)]
